...
namespace FMDemo.Features
{
    ...
    public sealed class LocalFeatures : DisposableCoreBase
    {
        ...
        public void AddOrSet(Guid key, string description)
        {
            ...
            if (key == Guid.Empty)
            {
                key = ToGuid(this.features.Count+1);
            }
        ...
        public void Save()
        {
            try
            {
                if (Directory.Exists(Path.GetDirectoryName(this.Filename)) == false)
                {
                    Directory.CreateDirectory(Path.GetDirectoryName(this.Filename));
                }

                string resultJson = JsonSerializer.Serialize<List<FeaturesContent>>(this.features);
            ...
            try
            {
                if (this.features != null)
                {
                    ...
                    if (File.Exists(this.Filename) == true)
                    {
                        ...
                        this.features = JsonSerializer.Deserialize<List<FeaturesContent>>(jsonString);
                    }
                }
            }
            catch (Exception ex)
